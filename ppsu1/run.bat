@echo off
setlocal
title PPSU PPT Designer - launcher
...
cls
echo ==================================================
echo               P P S U   P P T   D E S I G N E R
echo ==================================================
echo.
echo   On this computer : http://localhost:8001/
echo   Share on network : http://%LAN_IP%:8001/
echo.
echo ==================================================
echo.
...
set "SHORTCUT=Open PPSU PPT Designer.url"
> "%SHORTCUT%" echo [InternetShortcut]
>>"%SHORTCUT%" echo URL=http://%LAN_IP%:8001/
...
start "PPSU PPT Designer - BACKEND (do not close)" cmd /k "cd backend && python app.py"

REM ---------- start the frontend static server ----------
start "PPSU PPT Designer - FRONTEND (do not close)" cmd /k "python -m http.server 8001 --bind 0.0.0.0 --directory frontend"
