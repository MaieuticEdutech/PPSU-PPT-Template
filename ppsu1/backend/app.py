...
UPLOAD_DIR = Path(tempfile.gettempdir()) / "ppsu_ppt_engine_uploads"
UPLOAD_DIR.mkdir(exist_ok=True)

# Bundled design template, used whenever the user does not upload one of
# their own — uploading a template is optional. DEFAULT_TEMPLATE_PATH lets a
# hosted deployment point at a different bundled file (e.g. the small
# "PPSU TEMPLATE.pptx") without a code change — useful on memory-tight hosts,
# since the 50 MB library needs ~460 MB of RAM to open.
_default_override = os.environ.get("DEFAULT_TEMPLATE_PATH")
DEFAULT_TEMPLATE = (Path(_default_override) if _default_override
                    else APP_DIR.parent / "template" / "TEMPLATE FOR PPSU.pptx")


def _template_path(work: Path) -> Path:
    """The session's design template: the uploaded one if present, else the
    bundled default."""
    uploaded = work / "template.pptx"
...
def _run_designs(work: Path):
    ...
    template_path = _template_path(work)
    ...
    for i, raw_info in enumerate(raw_meta):
        ...
        if not slide_urls:
            # the deck itself is fine and downloadable — only its preview
            # could not be rendered. Say so instead of showing a blank page.
            skipped.append(
                f'{raw_info["filename"]} — the deck was rebuilt and can be '
                f'downloaded, but its preview could not be rendered. Click '
                f'Redesign to try the preview again.')
...
def _receive_upload():
    ...
    if not raw_files:
        return None, (jsonify({"error": "Missing raw files"}), 400)

    # the template is OPTIONAL: when none is uploaded the bundled PPSU
    # design template is used instead
    template_file = request.files.get("template")
    if (template_file is None or not template_file.filename) \
            and not DEFAULT_TEMPLATE.exists():
        return None, (jsonify({"error": (
            "No template was uploaded and the bundled default template is "
            "missing on the server.")}), 400)
...
def result(token):
    """The last finished build's preview payload for a session.

    Lets the page recover when the long /build request itself is dropped by a
    proxy (hosted free tiers build slowly): it polls /progress until the phase
    is "done", then fetches the payload here instead of re-building."""
    work = _work_dir(token)
    meta_path = work / "meta.json"
    if not meta_path.exists():
        return jsonify({"error": "No finished build for this session yet"}), 404
...
def _deck_templates(work: Path):
    ...
    catalog = json.loads(get_catalog(_template_path(work)).read_text())
    by_count = {}
...
if __name__ == "__main__":
    ...
    # network reach the API at http://<this-machine-LAN-IP>:5001
