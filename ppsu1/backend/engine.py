...
def _single_body_box(content, slide_shapes, decor):
    ...
    for s in others:
        ...
        # as the bullets, and one even SMALLER — 18 pt kicker over a 20 pt
        # body; a lone line above a full points box reads as a subtitle
        # whatever its size)
...
def _slot_icon_color(slot):
    ...
    if safe_shape_type(slot) == 13:  # PICTURE
...
def place_icon(slide, slot, icon):
    ...
    if png is None:
        return False
    try:
        if safe_shape_type(slot) == 13:  # PICTURE: swap the image, keep the frame
...
def _blank_layout(prs):
    ...
    def master_has_picture(lo):
        return any("PICTURE" in str(safe_shape_type(s))
