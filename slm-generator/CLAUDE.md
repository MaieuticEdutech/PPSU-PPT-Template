...
(schemas.FIGURE_SPEC — kind ∈ flow|cycle|hierarchy|bar_chart + 2-8
labelled items) and `backend/figure_render.py` draws it DETERMINISTICALLY
with matplotlib (Agg backend; PPSU navy/orange from styles.py; new
matplotlib dep in requirements.txt). Same division of labour as the docx
builder: the model never draws, code owns layout. Containment: a failed
spec call, an invalid spec, or a rendering error keeps the DTP
placeholder box — figures degrade, they never fail a unit
(report.figures = {planned, rendered}). The docx builder embeds
block["image"] (5.5in, centred, caption kept) with a _try_logo-style
belt; figures.txt now labels each figure [auto-rendered] (replaceable)
vs [PLACEHOLDER] (must be drawn). Wired everywhere via generate_unit's
`figures_dir` param: web job (work/figures), course mode
(figures_root/unitNN), CLI (out_stem_figures), batch. Live-verified on
the real 7B: correct kind selection for process/quantity/classification
captions, and REAL data in the bar chart (English letter frequencies
12.7/9.1/8.2 for E/T/A). One live lesson locked in: the model chose
bar_chart correctly but omitted numeric values → 'value' is now REQUIRED
in the schema for every item (constrained decoding enforces it
server-side; non-chart kinds ignore the number) — documented in
schemas.py. `tests/test_figures.py`: 25 checks (all kinds render valid
PNGs, six rejection paths, docx embed + fallback, DTP statuses, stubbed
generate_unit pass incl. containment and old-behaviour-without-
figures_dir) — 227 offline checks across nine suites.

**Full-course engine (N source docs → M units) added, 2026-09-04** (user
request: "take 5 units, generate 14 units, don't skip any of the
syllabus"). `backend/course_generator.py`: every uploaded document is
extracted and chunked (per-file provenance kept); the combined ordered
topic inventory goes to ONE planning call (`prompts.course_plan`, a
dynamically-built exactly-N-units schema, num_predict=4096) that returns
only unit TITLES and contiguous END cut-points. **Coverage is enforced in
code, never trusted from the model**: `_repair_cuts` clamps/orders the
proposed cut points (leaving room for every later unit), forces the final
cut to the last topic, and falls back to an equal split if the plan is
unusable — so every detected topic lands in exactly one unit, in the
sources' teaching order, regardless of model quality. Each unit is then
generated by the normal textbook pipeline via `generate_unit`'s new
`source_chunks`/`source_label` parameters (pre-chunked slice, skips
re-extraction; `source_present` now gates all provenance/citation logic
so chunk-fed units get the full grounding treatment). One unit failing
(crash OR validation) never stops the rest; `write_course_outputs`
renders per-unit docx(+pdf)/report/figure-list plus course_report.json
(plan, per-unit status, and a coverage proof block
{topics_total, topics_assigned, complete}) and zips everything. Web:
`POST /api/generate_course` (multi-file `sources`, `target_units` 2–30
default 14, same brand/level fields and _RUN_LOCK), progress rows
labelled "[unit i/N]", `GET /api/download/{job}/zip`; frontend "Full
course" card (multi-select file input + unit count) with a course result
view: coverage line, plan-repair notice, per-unit status list, ZIP +
course-report downloads. `_Caller` gained an optional num_predict
pass-through (forwarded only when set, so simple engine stubs stay
valid). `tests/test_course_generator.py`: 40 checks (cut-point repair
incl. greedy/garbage/one-topic-per-unit edges, full-coverage invariants
under both good and broken plans, provenance, failure containment,
ZIP contents, REVA naming, endpoint validation + job lifecycle) — 202
offline checks across eight suites.
